...
export function ReviewScreen() {
  ...
  // Configured TTS speed. Export applies this multiplier to TTS audio, so the
  // Review prediction must too — otherwise a user with Azure speed=1.2 would
  // see phantom "will compress" warnings that don't manifest at export time.
  // Loaded async from the persisted config for the provider Export will use.
  const exportTtsProvider = useConfigStore((s) => s.ttsProvider);
  const [exportTtsSpeed, setExportTtsSpeed] = useState(1.0);
  useEffect(() => {
    let cancelled = false;
    const loadSpeed = async () => {
      try {
        if (exportTtsProvider === "elevenlabs") {
          const cfg = await getElevenLabsConfig();
          if (!cancelled) setExportTtsSpeed(cfg?.speed || 1.0);
        } else if (exportTtsProvider === "azure") {
          const cfg = await getAzureTtsConfig();
          if (!cancelled) setExportTtsSpeed(cfg?.speed || 1.0);
        } else {
          // Builtin — speed is in localStorage, same key Export uses.
          try {
            const saved = localStorage.getItem("narrator_builtin_tts");
            const parsed = saved ? JSON.parse(saved) : null;
            if (!cancelled) setExportTtsSpeed(parsed?.speed || 1.0);
          } catch {
            if (!cancelled) setExportTtsSpeed(1.0);
          }
        }
      } catch {
        if (!cancelled) setExportTtsSpeed(1.0);
  ...
  // Per-segment speech-rate report. Recomputed whenever segments or language
  // change so text/timing edits update the badge + footer banner in real time.
  // We don't trust `script.speech_rate_report` after local edits — it was
  // generated by the server for the original text. Regenerating client-side is
  // cheap (< 1ms for 50 segments) and matches what Export will actually do.
  const speechReport = useMemo(
    () => computeSpeechRateReport(segments, activeLanguage, exportTtsSpeed),
    [segments, activeLanguage, exportTtsSpeed],
  );
  const exportPrediction = useMemo(
    () => predictExport(segments, activeLanguage, duration > 0 ? duration : (script?.total_duration_seconds ?? 0), exportTtsSpeed),
    [segments, activeLanguage, duration, script?.total_duration_seconds, exportTtsSpeed],
  );
                ...
                <div key={i} onClick={() => handleSegmentClick(i)} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 2, minWidth: 0 }}>
                      {speechReport[i] && <SegmentSpeechChip overflow={speechReport[i]} />}
                      {seg.visual_description && (
                        <p style={{ fontSize: 10, color: "#7a7a8e", margin: 0, fontStyle: "italic", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, minWidth: 0 }}>{seg.visual_description}</p>
                      )}
                    </div>
                  </div>
...
// ── Speech-rate UI helpers ────────────────────────────────────────────────

const SEVERITY_STYLES: Record<Severity, { bg: string; fg: string; border: string }> = {
  fit:      { bg: "transparent",              fg: "transparent",   border: "transparent" },
  tight:    { bg: "rgba(250,204,21,0.10)",    fg: "#facc15",       border: "rgba(250,204,21,0.35)" },
  compress: { bg: "rgba(249,115,22,0.12)",    fg: "#fb923c",       border: "rgba(249,115,22,0.35)" },
  overflow: { bg: "rgba(239,68,68,0.14)",     fg: "#f87171",       border: "rgba(239,68,68,0.45)" },
};

function SegmentSpeechChip({ overflow }: { overflow: { severity: Severity; predicted_seconds: number; window_seconds: number } }) {
  if (overflow.severity === "fit") return null;
  const s = SEVERITY_STYLES[overflow.severity];
  const predicted = overflow.predicted_seconds;
  const over = predicted - overflow.window_seconds;
  let label: string;
  let title: string;
  switch (overflow.severity) {
    case "tight":
      label = `~${predicted.toFixed(1)}s · tight`;
      title = `Narration will take about ${predicted.toFixed(1)}s in a ${overflow.window_seconds.toFixed(1)}s window — no breathing room.`;
      break;
    case "compress":
      label = `~${predicted.toFixed(1)}s · will speed up`;
      title = `Narration (~${predicted.toFixed(1)}s) slightly exceeds the ${overflow.window_seconds.toFixed(1)}s window. Export will speed this segment up to fit.`;
      break;
    case "overflow":
      label = `~${predicted.toFixed(1)}s · over by ${over.toFixed(1)}s`;
      title = `Narration (~${predicted.toFixed(1)}s) is far longer than the ${overflow.window_seconds.toFixed(1)}s window. Export will hold the last frame for ~${over.toFixed(1)}s so the sentence can finish — or you can shorten the text here.`;
      break;
    default:
      return null;
  }
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        background: "rgba(0,0,0,0.25)", color: fg, fontWeight: 700,
        display: "flex", alignItems: "center", justifyContent: "center",
