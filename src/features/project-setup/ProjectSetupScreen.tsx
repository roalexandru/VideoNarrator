...
import { probeVideo } from "../../lib/tauri/commands";
...
const inputStyle: CSSProperties = {
  ...
  fontSize: 14, background: C.inputBg, color: C.text,
...
export function ProjectSetupScreen() {
  ...
  const handleVideoSelect = useCallback(async () => {
    ...
    try {
      ...
      setVideoFile({
        path: m.path, name: m.path.split("/").pop() || "video",
        size: m.file_size, duration: m.duration_seconds,
        resolution: { width: m.width, height: m.height },
        codec: m.codec, fps: m.fps,
      });
    } catch (err) { console.error("Failed to probe video:", err); }
  }, [setVideoFile]);
  ...
  const handleDocSelect = useCallback(async () => {
    ...
    addDocuments(paths.map((p) => ({
      id: crypto.randomUUID(), path: p,
      name: p.split("/").pop() || "document", size: 0,
      type: (p.split(".").pop() || "txt") as "md" | "txt" | "pdf",
    })));
  }, [addDocuments]);
      ...
      <section style={{ marginBottom: 28 }}>
        ...
        {videoFile ? (
          ...
          >
            ...
            <div style={{ fontWeight: 600, color: C.textDim, fontSize: 14, fontFamily: "inherit" }}>Click to select a video file</div>
            <div style={{ color: C.textMuted, fontSize: 12, marginTop: 4, fontFamily: "inherit" }}>MP4, MOV, AVI, MKV, WebM</div>
          </button>
        )}
      </section>
      ...
      <section style={{ marginBottom: 28 }}>
        ...
        {contextDocuments.length > 0 ? (
              ...
              <div key={doc.id} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{
                    ...
                    borderRadius: 5, textTransform: "uppercase", fontFamily: "monospace",
                  }}>{doc.type}</span>
          ...
          <p style={{ fontSize: 13, color: C.textMuted, fontStyle: "italic" }}>
            Brand guides, product docs, or glossaries improve narration quality.
          </p>
        )}
      </section>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
              placeholder="e.g., UiPath Studio Walkthrough" style={inputStyle}
          ...
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)}
              placeholder="What should the viewer learn?" rows={3}
